import gradio as gr
from simple import query_by_vector
import os
import requests
import json
...
def rag_chat(user_message, system_prompt, top_k):
    if not user_message or not user_message.strip():
        yield [
            {"role": "assistant", "content": "⚠️ 请输入有效的问题内容。"},
            ""
        ]
        return
    
    rag_results = query_by_vector(user_message, top_k=top_k)
    context = "\n\n".join([f"片段{i+1}: {r[0]}" for i, r in enumerate(rag_results)])
    prompt = f"""
【开始上下文】
{context}
【结束上下文】
...
with gr.Blocks(title="RAG智能助手") as demo:
    gr.Markdown("# RAG智能助手（Ollama本地大模型）\n\n请输入您的问题，系统会结合知识库智能回答。")
    ...
    clear = gr.ClearButton([msg, chatbot, system_prompt_box, context_box, top_k_box])
